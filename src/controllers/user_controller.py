...
@router.get(path=LISTA_USUARIOS, response_model=List[UserResponse], tags=[Tag.Clientes.name])  # Resposta será uma lista de objetos UserResponse
...
@router.get(path=OBTER_POR_ID_USUARIO, response_model=UserResponse, tags=[Tag.Clientes.name])  # Resposta será um único objeto UserResponse
...
@router.post(path=CADASTRO_USUARIO, response_model=UserResponse, tags=[Tag.Clientes.name])  # A resposta será um único objeto UserResponse
...
@router.put(path=ATUALIZAR_USUARIO, response_model=UserResponse, tags=[Tag.Clientes.name])  # A resposta será um único objeto UserResponse
def update_user(user_id: int, user_update: UserUpdate, db: Session = Depends(get_db)):  # Recebe o ID e os dados para atualização
    user = db.query(User).filter(User.id == user_id).first()  # Consulta o usuário pelo ID
    if not user:
        raise HTTPException(status_code=404, detail="User not found")  # Lança exceção se o usuário não for encontrado
...
@router.delete(path=APAGAR_USUARIO, tags=[Tag.Clientes.name])  # Esse endpoint não retorna nenhum dado, apenas confirma a exclusão
